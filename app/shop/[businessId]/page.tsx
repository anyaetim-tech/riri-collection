...
type Product = { id: string; name: string; price: number; stock: number; image_url?: string; business_id: string };
...
export default function PublicShopPage({ params }: { params: { businessId: string } }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Products
      const { data: prods } = await supabase.from("products").select("id, name, price, stock, image_url, business_id").eq("business_id", businessId).gt("stock", 0).order("created_at", { ascending: false });
  ...
  async function placeOrder() {
    if (!customer.name || !customer.phone) {
      alert("Enter your name and WhatsApp number");
      return;
    }
    if (cart.length === 0) {
      alert("Cart empty");
    ...
    try {
      ...
      alert("Could not place order: " + e.message + " — Make sure you ran the SHOP_RLS.sql I will give you");
    } finally {
      setPlacing(false);
    }
  }
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4">
            {products.map(p => (
              <div key={p.id} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                <div className="h-32 rounded-xl bg-gray-100 flex items-center justify-center text-3xl">{p.image_url ? <img src={p.image_url} alt={p.name} className="h-full w-full object-cover rounded-xl"/> : "👗"}</div>
                <h3 className="mt-3 font-bold text-sm line-clamp-1">{p.name}</h3>
                <p className="text-xs text-gray-500">{p.stock} left</p>
            ...
            {products.length===0 && <p className="col-span-full text-sm text-gray-400">No products yet — vendor still uploading.</p>}
          </div>
        </div>
            ...
            {cart.length===0 ? <p className="mt-3 text-sm text-gray-400">Cart empty — add items from left.</p> : (
              <>
                <div className="mt-4 space-y-3">
                  {cart.map(c => (
                    <div key={c.id} className="flex justify-between text-sm"><span>{c.name} x{c.qty}</span><span className="flex items-center gap-2">₦{(c.price*c.qty).toLocaleString()}<button onClick={()=>removeFromCart(c.id)} className="text-red-500 text-xs">x</button></span></div>
                  ))}
                </div>
